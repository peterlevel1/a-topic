...
function getMainNode(target) {
	target = target || 'index';
	var mainNode;
	// console.log(packer.nodes[keys[0]]);
...
function createModule(bufs) {
	...
	var body = [];
	var main = bufs[bufs.length - 1].filename;
	var node;
	var i = 0;
	...
	body = packHelper.closureHead + '\n\n'
		...
		// + packHelper.isEmptyStr + '\n\n'
		+ body + ';\n'
		+ packHelper.setAll + '\n'
	  + packHelper.setNinja(keys.length - 1) + '\n'
...
File.prototype.setRequireArgs = function () {
	...
	requireArgs = requireArgs.map(function (one) {
		...
		if (path.isAbsolute(url)) {
			...
			var a = self.ifRequireOnlyName(url);
			if (!a) {
				console.log('not get it: ', url, self.filename);
			} else {
				return a;
			}
		}
	...
	if (this.requireArgs[0] === void 0) {
		// console.log(this.__requireArgs, this.filename);
	}
